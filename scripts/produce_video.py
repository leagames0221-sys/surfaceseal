...
def _scenes_factory() -> list[Scene]:
    ...
    return [
        Scene("S1", 9.0, s1, "エーアイ助手の設定変更に紛れた毒を、 世に出す前に止めるツール、 サーフィスシールです。"),
        Scene("S2", 9.5, s2, "エーアイ助手は設定ファイルを自動で信じて動きます。 そこに毒を仕込む攻撃が実際に起きています。"),
        ...
        Scene("S8", 9.5, s8, "毒入りの変更は終了コードでマージを止めます。 結果は標準形式でセキュリティ画面にも並びます。"),
